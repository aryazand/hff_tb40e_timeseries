...
rule run_multiqc:
    input:
        ...
        expand(os.path.join(FASTQC_PROCESSED_DIR, "{sample}_{direction}_trimmed_fastqc.html"), sample = sample_table["sample_name"], direction = ["1","2"]), 
        expand(os.path.join(FASTQC_PROCESSED_DIR, "{sample}_{direction}_trimmed_umi_fastqc.html"), sample = sample_table["sample_name"], direction = ["1","2"]), 
    output:
    ...
    log:
        ...
        err = "log/multiqc.err"
    shell:
        "multiqc results/qc/ --outdir $(dirname {output}) --force 2> {log.err} 1> {log.out}"
